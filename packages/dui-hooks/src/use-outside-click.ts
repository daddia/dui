...
// Helper functions for DOM element type checking
function isHTMLorSVGElement(element: EventTarget | null): element is HTMLElement & SVGElement {
    ...
    (event) => {
      ...
      return handleOutsideClick(event, () => {
        if (isHTMLorSVGElement(event.target)) {
          return event.target as HTMLElement & SVGElement;
        }
        return null;
      });
    },
