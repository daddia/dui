import { forwardRef, type ElementType, type RefObject, type ReactElement } from 'react';
import type { Props } from '@dui/types';
...
export function mergeRefs<T = unknown>(...refs: (RefObject<T> | ((instance: T) => void) | null)[]) {
  return (value: T): void => {
    refs.forEach((ref) => {
...
export function forwardRefWithAs<T extends { name: string; displayName?: string }>(
  ...
  return Object.assign(
    forwardRef(
      component as unknown as React.ForwardRefRenderFunction<unknown, unknown>,
    ) as unknown as T,
    {
